...
---
## 3. Выбор среды разработки и концептуальное описание модели

При выборе среды разработки, прежде всего, исходим из типа решаемой задачи, типов обрабатываемых данных, перспектив роста и масштабирования. Исходя из условиямпоставленной задачи, в качестве языка программирования лучше использовать Python. Основными преимуществами данного языка являются:
...
Структура модели содержит класс со следующими компонентами:

*   Инициализации объекта;
*   Расчет характеристик тела в каждый момент времени;
*   Отрисовка объекта в модальном окне;

Тип объектов разработан с учетом дальнейшего расширения модели, наприме добавление солнечного ветра, атмосферы плане и.т.д.

Структура модели представлена на рисунке ниже

![Структура модели](1.png)
...
Параметр  |  | Тип | Возможное значение параметра | Описание параметра 
------ | ------ | ------ | ------ | ------
Object | | | | 
Type |  | float | 1 | Тип объекта 
_ | CoordinateX | float | 0 | Координата начального положения Солнца по оси абсцисс (м)
_ | CoordinateY | float | 0 | Координата начального положения Солнца по оси ординат (м)
_ | Radius | float | 696000000 | Радиус Солнца (м)
_ | Weight | float | 1.9891*10**30 | Масса Солнца (кг) 
_ | SpeedX | float | 0 | Начальная скорость Солнца по оси абсцисс (м/с) 
_ | SpeedY | float | 0 | Начальная скорость Солнца по оси ординат (м/с)
_ | Colour | str | yellow | Цвет отображения в модели 
_ | Name | str | Sun | Имя объекта
Object | | | | 
Type |  | float | 2 | Тип объекта 
_ | CoordinateX | float | -149.6*10**9 | Координата начального положения Земли по оси абсцисс 
_ | CoordinateY | float | 1 | Координата начального положения Земли по оси ординат 
_ | Radius | float | 6371000 | Радиус Земли (м)
_ | Weight | float | 5.9722*10**24 | Масса Земли (кг) 
_ | SpeedX | float | 0 | Начальная скорость Земли по оси абсцисс (м/с) 
_ | SpeedY | float | 29782.77 | Начальная скорость Земли по оси ординат (м/с)
_ | Colour | str | blue | Цвет отображения в модели 
_ | Name | str | Earth | Имя объекта
Object | | | | 
Type |  | float | 3 | Тип объекта 
_ | CoordinateX | float | -149.6*10**9+123104000 | Координата начального положения Астероида по оси абсцисс 
_ | CoordinateY | float | 149.6*10**9+1231040000 | Координата начального положения Астероида по оси ординат 
_ | Radius | float | 463500 | Радиус Астероида (м)
_ | Weight | float | 9.393*10**20 | Масса Астероида (кг) 
_ | SpeedX | float | 3000 | Начальная скорость Астероида по оси абсцисс (м/с) 
_ | SpeedY | float | 29782.77 | Начальная скорость Астероида по оси ординат (м/с)
_ | Colour | str | red | Цвет отображения в модели 
_ | Name | str | Asteroid | Имя объекта
Object | | | | 
Type |  | float | 4 | Тип объекта 
_ | CoordinateX | float | -149.6*10**9+363104000 | Координата начального положения Луны по оси абсцисс 
_ | CoordinateY | float | 0 | Координата начального положения Луны по оси ординат 
_ | Radius | float | 1737100 | Радиус Луны (м)
_ | Weight | float | 7.35*10**22 | Масса Луны (кг) 
_ | SpeedX | float | 0 | Начальная скорость Луны по оси абсцисс (м/с) 
_ | SpeedY | float | 29782.77+1080 | Начальная скорость Луны по оси ординат (м/с)
_ | Colour | str | grey | Цвет отображения в модели 
_ | Name | str | Moon | Имя объекта
Object | | | | 
Type |  | float | 5 | Тип объекта 
_ | CoordinateX | float | 69.6*10**9 | Координата начального положения Планеты по оси абсцисс 
_ | CoordinateY | float | 30*10**9 | Координата начального положения Планеты по оси ординат 
_ | Radius | float | 6371000 | Радиус Планеты (м)
_ | Weight | float | 5.9722*10**24 | Масса Планеты (кг) 
_ | SpeedX | float | 0 | Начальная скорость Планеты по оси абсцисс (м/с) 
_ | SpeedY | float | 27782.77 | Начальная скорость Планеты по оси ординат (м/с)
_ | Colour | str | brown | Цвет отображения в модели 
_ | Name | str | Planet | Имя объекта
Object | | | | 
Type |  | float | 6 | Тип объекта 
_ | CoordinateX | float | -778570000000 | Координата начального положения Планеты по оси абсцисс 
_ | CoordinateY | float | 0 | Координата начального положения Юпитера по оси ординат 
_ | Radius | float | 71492000 | Радиус Юпитера (м)
_ | Weight | float | 1.8986*10**27 | Масса Юпитера (кг) 
_ | SpeedX | float | 0 | Начальная скорость Юпитера по оси абсцисс (м/с) 
_ | SpeedY | float | 13070 | Начальная скорость Юпитера по оси ординат (м/с)
_ | Colour | str | orange | Цвет отображения в модели 
_ | Name | str | Jupiter | Имя объекта
...
2. Техническая часть: в файл записываются координаты (x, y) точек траекторий всех объектов, существующих в модели, в количестве, равном длине массива точек траектории, а также текущее положение объекта. При закрытии программы файл сохраняет последние записанные значения точек траекторий и положений самих объектов.
...
В данной модели в общем виде рассматривается "газовые гиганты" и планеты "земного типа".
...
$\rho$ — плотность атмосферы на орбите, которая сильно зависит от высоты полета объекта, в данной модели рассчитывается как:
...
В базовой модели влияние атмосферы на движение объектов не учитывается (силой сопротивления пренебригаем).
...
В базовой реализации модели центр масс находится в геометрическом центре модели. В расширенной реализации центр масс задается вручную. В дальнейшем все формулы для гравитационного взаимодействия расчитываются от центра масс. 
...
В базовой реализации модели данные силы не учитываются. В расширенной реализации вручную задается интенсивность излучения, коэфициент пропускания, коэффициент отражения (для расчета давления света), а также средняя концетрация протонов (для расчета силы солнечного ветра). Обе силы напрвалены от солнца, они складываются покоординатно с равнодействующей силы гравитации.
